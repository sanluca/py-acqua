...
void MD5::Append (md5_state_t *ctx, unsigned char *input, unsigned int inputLen)
{
	unsigned int i, index, partLen;
	
	index = (unsigned int)((ctx->count[0] >> 3) & 0x3F);
	
	if ((ctx->count[0] += ((unsigned long int)inputLen << 3)) < ((unsigned long int)inputLen << 3))
		ctx->count[1]++;
...
void MD5::Finish (unsigned char digest[16], md5_state_t *ctx)
{
...
void MD5::Transform (unsigned long int state[4], unsigned char block[64])
{
...
void MD5::Decode (unsigned long int *output, unsigned char *input, unsigned int len)
{
	...
	for (i = 0, j = 0; j < len; i++, j += 4)
		output[i] =
		((ulong)input[j]) | (((ulong)input[j+1]) << 8) | (((ulong)input[j+2]) << 16) | (((ulong)input[j+3]) << 24);
}
